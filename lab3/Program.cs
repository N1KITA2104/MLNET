...
class Program
{
    static void Main(string[] args)
    {
        ...
        Console.WriteLine("=== Laboratory Work 3: Anomaly Detection ===");
        Console.WriteLine("CPU Metrics Anomaly Detection\n");
        ...
        if (args.Length > 0)
        {
            ...
            {
                AnomalyModelTrainer.TrainAndSaveModel();
            }
            else if (mode == "predict" || mode == "2")
            {
                AnomalyModelPredictor.LoadAndPredict();
            }
            else
            {
                ShowUsage();
